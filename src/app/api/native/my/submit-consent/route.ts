...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!base64) {
      return NextResponse.json({ error: "서명을 다시 해주세요." }, { status: 400 });
    }
    ...
    if (insertError) {
      console.error("[submit-consent] insert failed:", insertError.code, insertError.message);
      return NextResponse.json(
        { error: "제출에 실패했어요. 잠시 후 다시 시도해 주세요." },
        { status: 500 },
      );
    }
    ...
    console.error("[submit-consent]", error instanceof Error ? error.message : "unknown error");
    return NextResponse.json(
      { error: "제출에 실패했어요. 잠시 후 다시 시도해 주세요." },
      { status: 500 },
    );
