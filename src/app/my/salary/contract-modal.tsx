...
export function ContractModal({ record, worker, hasProfile }: { record: WorkRecord; worker: WorkerInfo; hasProfile: boolean }) {
  ...
  return (
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent showCloseButton={false} className={`${step === "confirm" ? "max-w-sm" : "!max-w-4xl"} max-h-[90vh] overflow-y-auto p-0 gap-0`}>
        <VisuallyHidden><DialogTitle>급여 신청</DialogTitle></VisuallyHidden>
        {step !== "confirm" && (
          <button
        ...
        {step === "confirm" ? (
          <div className="p-6 space-y-4 text-center">
            <h2 className="text-lg font-bold">급여 신청</h2>
            <p className="text-sm text-muted-foreground leading-relaxed">
              근로계약서 작성하면 급여신청이 완료됩니다.<br />
              근로계약서 작성하기로 이동하시겠습니까?
            </p>
            <div className="flex gap-2 pt-2">
              <Button variant="outline" className="flex-1" onClick={() => setOpen(false)}>
                취소
              </Button>
              <Button className="flex-1 bg-[#1e2a5a] hover:bg-[#2a3a7a]" onClick={() => setStep("contract")}>
                이동하기
              </Button>
            </div>
          </div>
                ...
                <div className="space-y-4 p-4 text-sm leading-relaxed text-gray-700">
                  ...
                  <div>
                    ...
                    <div className="mt-2 overflow-hidden rounded border">
                      ...
                      <div className="flex border-b text-xs">
                        <div className="w-24 shrink-0 bg-gray-100 px-2 py-1.5 font-medium border-r">{wageType === "일급" ? "기본일급" : "기본시급"}</div>
                      ...
                      <div className="flex text-xs">
                        ...
                        <div className="px-2 py-1.5">{wageType === "일급" ? "기본일급=1일임금" : "기본시급x(근무시간-공제시간)=1일임금"}</div>
                      </div>
                    </div>
                    <p className="mt-2 text-xs text-gray-500">{wageType === "일급" ? "※ 기본일급은 홈페이지에 공지된 일급이며 별도의 안내를 받으신분은 따로 기입후 지급됨" : "※ 기본시급은 홈페이지에 공지된 시급이며 별도의 안내를 받으신분은 따로 기입후 지급됨"}</p>
