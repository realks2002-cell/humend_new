...
import { CheckCircle, Loader2, User, Phone, Lock, Eye, EyeOff, MapPin, Shield, ExternalLink, ShieldCheck, Check } from "lucide-react";
...
function KakaoLink({ children }: { children: ReactNode }) {
  const handleClick = async (e: MouseEvent<HTMLAnchorElement>) => {
    ...
    await Browser.open({ url: KAKAO_CHAT_URL });
  };

  return (
    <a href={KAKAO_CHAT_URL} target="_blank" rel="noopener noreferrer" className="underline" onClick={handleClick}>
      {children}
    </a>
  );
}
...
export default function SignupPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      const { status, data } = await postJson(`${API_BASE}/api/phone-verification/send`, { phone: reqPhone });
      const t = Date.now();
      setNow(t);
  ...
  const handleVerify = async () => {
    ...
    try {
      const { status, data } = await postJson(`${API_BASE}/api/phone-verification/verify`, {
        requestId,
        phone: sentPhone,
        code,
      });
  ...
  const handleSignup = async () => {
    ...
    try {
      const result = await memberSignup(sentPhone, name, password, requestId);
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  className={cn("pl-10", isVerified && "opacity-60")}
                  value={phone}
