...
import { memberLogin } from "@/lib/native-api/auth";
...
function PasswordReset({ onBack }: { onBack: () => void }) {
  ...
  const handleSend = async () => {
    ...
    try {
      const { status, data } = await postJson(`${API_BASE}/api/phone-verification/send`, { phone: reqPhone, purpose: "reset" });
      const t = Date.now();
      setNow(t);
  ...
  const handleReset = async () => {
    ...
    try {
      const { status, data } = await postJson(`${API_BASE}/api/auth/password-reset`, {
        phone: sentPhone,
