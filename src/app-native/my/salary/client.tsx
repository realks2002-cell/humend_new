...
import { AlertCircle, CreditCard, Wallet, Loader2 } from "lucide-react";
...
function SalaryContent() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getMyProfile(),
      getAllClients(),
    ]).then(([recordsData, profileData, clientsData]) => {
  ...
  const worker = {
    name: profile?.name ?? "",
    rrnFront:
      ((profile as unknown as Record<string, unknown>)?.rrn_front as string) ??
      "",
    rrnBack:
      ((profile as unknown as Record<string, unknown>)?.rrn_back as string) ??
      "",
    phone: profile?.phone ?? "",
    region:
    ...
    <div className="mx-auto max-w-3xl px-4 py-8 space-y-6">
          ...
          <div className="flex items-center justify-between rounded-xl border bg-white/80 px-4 py-3 text-sm">
            <span className="flex items-center gap-3">
              ...
              <span className="text-muted-foreground">
                {profile?.account_number
                  ? formatAccount(profile.account_number)
                  : "-"}
              </span>
              <span className="font-medium">
                {profile?.account_holder ?? profile?.name ?? "-"}
              </span>
            </span>
            <Link href="/my/resume">
              <Button
                size="sm"
                className="h-7 text-[13px] shrink-0 rounded-none bg-blue-600 text-white hover:bg-blue-700"
              >
                수정
              </Button>
            </Link>
                  ...
                  <CardContent className="flex items-center justify-between p-4">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1.5 flex items-center gap-1.5">
                        <Badge
                          className={`text-[10px] font-semibold border-0 ${statusStyle[displayStatus] ?? "bg-muted text-muted-foreground"}`}
                        >
                          {displayStatus}
                        </Badge>
                        {signed && (
      ...
      {pendingRecords.length > 0 && (
        <div className="space-y-3">
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider px-1">
            급여 신청 완료{" "}
            <Badge className="ml-1 bg-amber-500/10 text-amber-700 text-[10px] font-semibold border-0">
              {pendingRecords.length}건 대기
            </Badge>
          </h2>
          <div className="space-y-3">
            {pendingRecords.map((r) => (
              <Card
                key={r.id}
                className="overflow-hidden transition-all py-0 border-amber-200/80"
              >
