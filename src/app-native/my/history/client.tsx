...
import { ContractViewModal } from "@/app/my/salary/contract-view-modal";
import { PayslipModal } from "@/app/my/salary/payslip-modal";
...
function HistoryContent() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      const [allRecords, memberProfile] = await Promise.all([
      ...
      const filtered = allRecords.filter((r) => r.payments);

      // 서명 이미지 URL 가져오기
      const signed = filtered.filter((r) => r.signature_url);
  ...
  const totalHours = records.reduce(
    (s, r) => s + Number(r.payments!.work_hours) + Number(r.payments!.overtime_hours),
    0
  );
    ...
    <div className="mx-auto max-w-3xl px-4 py-8 pb-32 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        {statCards.map((stat) => (
          <div
            key={stat.label}
            className="relative overflow-hidden rounded-2xl border bg-card px-3 py-2.5"
          >
            <div
